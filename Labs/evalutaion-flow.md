...
In this lab, you will set up and analyze evaluation flows for an AI model using Microsoft Foundry. You will manually review model responses to various inputs, providing a detailed evaluation of the model's performance. By setting up specific evaluation metrics, such as coherence and fluency, you will automate the evaluation process using a provided dataset. This hands-on experience will help you understand how to critically assess and refine AI model outputs, ensuring the model meets desired performance standards.
...
1. A new window opens with your previous **System message (1)** already populated and your deployed **Model (2)** selected.

1. In the **Manual evaluation result (3)** section, you'll add five inputs for which you will review the output. Enter the following five questions as five separate inputs by selecting **+ Add Inputs (4)**:
...
1. You can now manually review the outputs for each question by selecting the **thumbs up / down** icon at the bottom right of a response. Rate each response, ensuring you include **at least one thumbs up** and **one thumbs down** response in your ratings.
...
1. Using the menu on the left, navigate to **Evaluation (1)**.

1. Select the **Manual evaluations (2)** tab to find the **manual evaluations you just saved (3)**. Note that you can explore your previously created manual evaluations, continue where you left off, and save the updated evaluations.
...
1. On create a new evaluation, select **Evaluate an existing query-response dataset (1)**, and click on **Next (2)**.
   ...
   ![](./media/evtg.png)
    ...
    - Search for **visual studio code (1)** in the Windows search bar of the vm and open **Visual Studio Code (2)**.
...
1. Navigate back to **Microsoft Foundry**, where you were **creating a new evaluation**.

   - **Configure test data**: select **Upload new dataset(1)** , then select the file that you downloaded earlier in this task **(sample.jsonl)** and click on **Next (2)**.
...
1. Under the **Report (1)** tab, scroll down to explore the **Metric dashboard (2)**.
